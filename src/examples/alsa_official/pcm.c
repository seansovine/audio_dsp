...
static void generate_sine(const snd_pcm_channel_area_t *areas, snd_pcm_uframes_t offset, int count,
                          double *_phase) {
    ...
    for (chn = 0; chn < channels; chn++) {
        ...
        samples[chn] =
            /*(signed short *)*/ (((unsigned char *)areas[chn].addr) + (areas[chn].first / 8));
...
static void async_callback(snd_async_handler_t *ahandler) {
    ...
    struct async_private_data *data =
        (struct async_private_data *)snd_async_handler_get_callback_private(ahandler);
...
int main(int argc, char *argv[]) {
    struct option long_option[] = {
        {"help", 0, NULL, 'h'},     {"device", 1, NULL, 'D'},     {"rate", 1, NULL, 'r'},
        {"channels", 1, NULL, 'c'}, {"frequency", 1, NULL, 'f'},  {"buffer", 1, NULL, 'b'},
        {"period", 1, NULL, 'p'},   {"method", 1, NULL, 'm'},     {"format", 1, NULL, 'o'},
        {"verbose", 1, NULL, 'v'},  {"noresample", 1, NULL, 'n'}, {"pevent", 1, NULL, 'e'},
        {NULL, 0, NULL, 0},
    };
    snd_pcm_t *handle;
    int err, morehelp;
